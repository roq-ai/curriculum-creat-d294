...
import { useRoqClient, useProposalFindManyWithCount } from 'lib/roq';
...
export function ProposalListPage(props: ProposalListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useProposalFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.proposal.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'description', header: 'Description', accessorKey: 'description' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.submitted_at ? format(record?.submitted_at, 'dd-MM-yyyy') : '',
    },
