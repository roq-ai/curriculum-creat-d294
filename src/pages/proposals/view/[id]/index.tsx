...
import { useProposalFindFirst, useRoqClient } from 'lib/roq';
...
function ProposalViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; content: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useProposalFindFirst(queryParams, {}, { disabled: !id });
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Submitted At"
                  text={data?.submitted_at ? format(data?.submitted_at, 'dd-MM-yyyy') : ''}
                />
