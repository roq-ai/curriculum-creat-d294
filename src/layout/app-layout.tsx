...
import { useInstitutionFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['school-administrator', 'content-creator', 'curriculum-reviewer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useInstitutionFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Institutions',
      path: isTenantUser ? `/institutions/view/${data?.id}` : '/institutions',
      entity: 'institution',
